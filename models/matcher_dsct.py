...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            out_prob = outputs["pred_logits"].flatten(0, 1).sigmoid()[:,:-1] # [batch_size * num_queries, num_class]
            out_bbox = outputs["pred_boxes"].flatten(0, 1)  # [batch_size * num_queries, 4]

            # Also concat the target labels and boxes
            tgt_cls = torch.cat([v["labels"] for v in targets]) # [batch_size * num_target_boxes, num_class]
            ...
            if self.binary_flag:
                ...
                cost_class = ((pos_cost_class - neg_cost_class) * tgt_cls).mean(-1)
            else:
                alpha = 0.25
                gamma = 2.0
                neg_cost_class = (1 - alpha) * (out_prob ** gamma) * (-(1 - out_prob + 1e-8).log())
