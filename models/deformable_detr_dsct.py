...
from .matcher_dsct import build_matcher
from .segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss, sigmoid_focal_loss, ce_loss)
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
                 binary_flag=False, aux_loss=True, with_box_refine=False, two_stage=False):
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_classes = []
        outputs_coords = []
        for lvl in range(hs_all.shape[0]):
            if lvl == 0:
                reference = init_reference_all
            else:
                reference = inter_references_all[lvl - 1]
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25, eos_coef=0.1, binary_flag=0):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        idx = self._get_src_permutation_idx(indices) # bs_idx, src_idx [batch_size * num_target_boxes, 2]

        if self.binary_flag:
            target_classes = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)]) # [batch_size * num_target_boxes, num_class]
            ...
            target_classes_onehot = torch.zeros([src_logits.shape[0], src_logits.shape[1], src_logits.shape[2]],
                                        dtype=target_classes.dtype, layout=target_classes.layout, device=target_classes.device)
            target_classes_onehot[:,:,-1] = 0
            ...
            loss_ce = sigmoid_focal_loss(src_logits[:,:,:-1], target_classes_onehot[:,:,:-1], num_boxes, alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
        ...
        if log:
            ...
            if self.binary_flag:
                # mAP = average_precision_score(target_classes.cpu().data.numpy(), src_logits[idx].cpu().data.numpy())
                # losses['class_error'] = torch.tensor(mAP, device=target_classes.device)
                losses['class_error'] = torch.tensor(0, device=target_classes.device)
            else: 
                # acc = accuracy_score(target_classes_o.cpu().data.numpy(), src_logits[idx].cpu().data.numpy())
                # losses['class_error'] = torch.tensor(acc, device=target_classes_o.device)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        topk_values, topk_indexes = torch.topk(prob.view(out_logits.shape[0], -1), 1, dim=1)
        scores = topk_values
...
def build(args):
    ...
    exec('from .{} import build_deforamble_transformer'.format(args.model), globals())

    transformer = globals()['build_deforamble_transformer'](args)
    ...
    weight_dict = {'loss_ce': args.cls_loss_coef, 'loss_em': args.em_loss_coef, 'loss_bbox': args.bbox_loss_coef}
    ...
    criterion = SetCriterion(num_classes, matcher, weight_dict, losses, binary_flag=args.binary_flag, focal_alpha=args.focal_alpha)
    criterion.to(device)
